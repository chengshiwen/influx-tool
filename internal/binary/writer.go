...
import (
	...
	"github.com/chengshiwen/influx-tool/internal/tlv"
...
func NewWriter(w io.Writer, db, rp string, sd time.Duration) *Writer {
	var wr *bufio.Writer
	if wr, _ = w.(*bufio.Writer); wr == nil {
		wr = bufio.NewWriter(w)
	}
	return &Writer{w: wr, db: db, rp: rp, sd: sd}
}
...
func (w *Writer) NewBucket(start, end int64) (*BucketWriter, error) {
...
func (bw *BucketWriter) WriteSeries(name []byte, field []byte, fieldType influxql.DataType, tags models.Tags, ci *storage.CursorIterator) error {
	bw.BeginSeries(name, field, fieldType, tags)
